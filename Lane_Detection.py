...
def RGB_color_selection(image):
    ...
    """
    #White color mask
    lower_threshold = np.uint8([119,111,77])
    upper_threshold = np.uint8([255, 255, 255])
    white_mask = cv2.inRange(image, lower_threshold, upper_threshold)
    
    #Yellow color mask
    lower_threshold = np.uint8([68,55,39])
    upper_threshold = np.uint8([255, 255, 255])
...
def HSV_color_selection(image):
    ...
    #White color mask
    lower_threshold = np.uint8([106,97,88])
...
def region_selection_night(image):
    """
    Adjust the region of interest dynamically for night videos.
    """
    ...
    # You may need to adjust the region dynamically based on the lighting conditions
...
list_images(list(map(region_selection_night, test_images)))


# In[12]:


def RGB_color_selection_night(image):
    """
    Adjust color selection for night videos in the RGB format.
    """
    lower_threshold = np.uint8([100, 100, 100])  # Adjust these thresholds based on the characteristics of your night video
    upper_threshold = np.uint8([255, 255, 255])
    white_mask = cv2.inRange(image, lower_threshold, upper_threshold)

    # Yellow color mask (you may need to fine-tune these values)
    lower_threshold = np.uint8([50, 50, 0])
...
list_images(list(map(RGB_color_selection_night, test_images)))


# In[14]:


def canny_detector_night(image, low_threshold=30, high_threshold=100):
    """
    Adjust Canny Edge Detection parameters for night videos.
    """
...
list_images(list(map(canny_detector_night, test_images)))
...
edge_detected_night_images = list(map(canny_detector_night, blur_images))
list_images(edge_detected_night_images)
...
masked_image_night = list(map(region_selection_night, edge_detected_images))
list_images(masked_image_night)
...
def draw_lines(image, lines, color=[255, 0, 0], thickness=2):
    ...
    Parameters:
        image: Input image.
        lines: List of lines, where each line is represented by (x1, y1, x2, y2).
        color (Default = [255, 0, 0]): Line color in RGB format.
...
def frame_processor(image):
    ...
    edges        = canny_detector_night(smooth)
    region       = region_selection_night(edges)
...
get_ipython().run_line_magic('time', "process_video('test.mp4', 'test_output.mp4')")
HTML("""
